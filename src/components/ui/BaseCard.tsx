...
const BaseCard = memo(
  forwardRef<HTMLDivElement, BaseCardProps>(function BaseCard(
    { children, interactive = false, isActive = false, wrapperClassName, className, ...motionProps },
    ref,
  ) {
    return (
      <motion.div
        ref={ref}
        className={cn("group relative h-full", interactive && "cursor-pointer", wrapperClassName)}
